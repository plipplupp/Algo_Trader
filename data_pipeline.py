...
def _add_momentum_indicators(df):
    ...
    for n in [9, 14, 21]:
        ...
        df[f'stoch_k_{n}'] = 100 * ((df['close'] - low_n) / (high_n - low_n))
    ...
    for n in [5, 10, 21, 63]:
        df[f'roc_{n}'] = df['close'].pct_change(periods=n, fill_method=None) * 100
    return df
...
def _add_volatility_indicators(df):
    ...
    for n in [10, 20, 50, 100]:
        df[f'volatility_{n}'] = df['close'].pct_change(fill_method=None).rolling(n).std() * np.sqrt(252)
    return df

def _add_volume_indicators(df):
    df['obv'] = (np.sign(df['close'].diff()) * df['volume']).fillna(0).cumsum()
    epsilon = 1e-10
    mfv = ((df['close'] - df['low']) - (df['high'] - df['close'])) / (df['high'] - df['low'] + epsilon)
...
def _add_advanced_and_interaction_features(df):
    ...
    if 'bb_upper_20' in df.columns:
        df['close_vs_bb_upper'] = df['close'] / df['bb_upper_20']
        df['close_vs_bb_lower'] = df['close'] / df['bb_lower_20']
        ...
        for col in feature_columns:
            if col in final_df.columns and final_df[col].isnull().any():
                if 'rsi' in col.lower():
                    ...
                    final_df[col].fillna(50, inplace=True)
